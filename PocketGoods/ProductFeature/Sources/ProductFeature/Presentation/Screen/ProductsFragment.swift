...
public struct ProductsFragment: View {
            ...
            if state.isPageLoading && state.products.isEmpty {
                ...
                default:
                    LazyVGrid(
                        ...
                        ForEach(state.products, id: \.id) { productPreview in
                            ProductCard(
                                product: productPreview,
                                isAuthenticated: authenticatedUser != nil,
                                inWishlist: authenticatedUser?
                                    .inWishlist(productId: productPreview.id)
                                    ?? false,
                            ...
                            .padding(.all, 8)
                            .onAppear {
                                if productPreview.id == state.products.last?.id
                                    && !state.isPageLoading
                                {
                                    onEvent(.onNextPage)
                                    onEvent(.getProducts)
...
private struct ProductsFragmentPreview: View {
